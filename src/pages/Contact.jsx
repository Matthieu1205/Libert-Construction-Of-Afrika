...
const PhoneSVG = (<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 9.81 19.79 19.79 0 01.01 1.18 2 2 0 012 0h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.09 7.91a16 16 0 006 6z"/></svg>);
const PinSVG  = (<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>);
const MailSVG = (<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>);
const ClockSVG= (<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><polyline points="12,6 12,12 16,14"/></svg>);
...
export default function Contact() {
  ...
  const INFO = [
    { icon: PinSVG,   label:'Adresse',            value: contact.address,   sub: contact.city,      href: null },
    { icon: PhoneSVG, label:'WhatsApp / Appel',    value: contact.whatsapp,  sub: 'Aussi disponible sur WhatsApp', href: contact.whatsapp_href },
    { icon: PhoneSVG, label:'Mobile',              value: contact.mobile,    sub: '',                href: contact.mobile_href },
    { icon: PhoneSVG, label:'Fixe',                value: contact.fixe,      sub: '',                href: contact.fixe_href },
    { icon: MailSVG,  label:'Email',               value: contact.email,     sub: 'Réponse sous 24h', href: `mailto:${contact.email}` },
    { icon: ClockSVG, label:'Horaires',            value: contact.hours,     sub: contact.hours_sub, href: null },
  ];
              ...
              {INFO.map((c, i) => (
                <div className="contact-info__item" key={i}>
                  ...
                  <div>
                    ...
                    {c.sub && <p className="contact-info__sub">{c.sub}</p>}
                  </div>
                </div>
              ))}
            </div>
            <a href={waLink} target="_blank" rel="noopener noreferrer" className="contact-whatsapp">
